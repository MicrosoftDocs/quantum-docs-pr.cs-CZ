...
Jako jazyk a jako platforma Q# umožňuje uživatelům psát, spouštět, rozumět a prozkoumat aplikace s využitím více procesorů.
Aby bylo možné uživatele při návrhu Q# knihoven, pořiďte si sadu principů návrhu rozhraní API, které vám pomohou s návrhy a pomáhat při vytváření použitelných knihoven pro komunita pro vývoj všech stavů.
Tento článek obsahuje seznam těchto principů a popisuje příklady, které vám pomůžou při navrhování Q# rozhraní API použít.

> [!TIP]
> Toto je poměrně podrobný dokument, který je určený k tomu, aby usnadnil vývoj knihovny a podrobné příspěvky knihoven.
> Je pravděpodobné, že je to nejužitečnější, pokud píšete vlastní knihovny v nástroji Q# , nebo pokud přispíváte do [ Q# úložiště knihoven](https://github.com/microsoft/QuantumLibraries)větší funkce.
>
...
> Pokud hledáte obecnější informace o tom, jak kód Doporučujeme formátovat Q# , může vás zajímat [Průvodce stylem](xref:microsoft.quantum.contributing.style).
...
- ✅**Udělejte** návrh funkcí a operací, abyste mohli dobře začínat dalšími funkcemi a operacemi ve stejném rozhraní API i v dříve existujících knihovnách.
  ...
  *Příklady:*
  - @"microsoft.quantum.canon.delay"Operace vytváří minimální předpoklady o jeho vstupu, takže se dá použít ke zpoždění aplikací obou operací v rámci Q# standardní knihovny nebo definované uživateli.
...
- ✅**Vyberte operace** a názvy funkcí, které jasně sdělují zamýšlený účinek, který lze volat, a nikoli jeho implementaci. Všimněte si, že implementace může a měla by se zdokumentovat v [dokumentaci k dokumentaci k rozhraní API](xref:microsoft.quantum.qsharp.comments#documentation-comments).
