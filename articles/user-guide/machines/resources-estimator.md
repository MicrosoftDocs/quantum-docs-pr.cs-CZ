...
description: 'Přečtěte si o QDK prostředcích Microsoft Estimator, které vyodhadují prostředky potřebné ke spuštění dané instance :::no-loc(Q#)::: operace na počítači s více operačními systémy.'
author: anpaz-msft
ms.author: anpaz
ms.date: 06/26/2020
ms.topic: article
...
<span data-ttu-id="cf10d-104">Jak název implikuje, `ResourcesEstimator` Třída odhadne prostředky potřebné ke spuštění dané instance :::no-loc(Q#)::: operace v počítači s více operačními systémy.</span><span class="sxs-lookup"><span data-stu-id="cf10d-104">As the name implies, the `ResourcesEstimator` class estimates the resources required to run a given instance of a :::no-loc(Q#)::: operation on a quantum computer.</span></span> <span data-ttu-id="cf10d-105">To dosahuje tím, že spustí operaci bez skutečného simulace stavu počítače s více operačními systémy. z tohoto důvodu odhaduje prostředky pro :::no-loc(Q#)::: operace, které používají tisíce qubits, za předpokladu, že klasická část kódu běží v rozumnou dobu.</span><span class="sxs-lookup"><span data-stu-id="cf10d-105">It accomplishes this by running the quantum operation without actually simulating the state of a quantum computer; for this reason, it estimates resources for :::no-loc(Q#)::: operations that use thousands of qubits, provided that the classical part of the code runs in a reasonable time.</span></span>

<span data-ttu-id="cf10d-106">Prostředky Estimator jsou postavené na [simulátoru trasování](xref:microsoft.quantum.machines.qc-trace-simulator.intro)pro stav, který poskytuje bohatší sadu metrik a nástrojů pro ladění :::no-loc(Q#)::: programů.</span><span class="sxs-lookup"><span data-stu-id="cf10d-106">The resources estimator is built on top of the [Quantum trace simulator](xref:microsoft.quantum.machines.qc-trace-simulator.intro), which provides a richer set of metrics and tools to help debug :::no-loc(Q#)::: programs.</span></span>
...
<span data-ttu-id="cf10d-108">Pomocí prostředků Estimator můžete spustit jakoukoli :::no-loc(Q#)::: operaci.</span><span class="sxs-lookup"><span data-stu-id="cf10d-108">You can use the resources estimator to run any :::no-loc(Q#)::: operation.</span></span> <span data-ttu-id="cf10d-109">Další podrobnosti najdete v tématu [způsoby spuštění :::no-loc(Q#)::: programu](xref:microsoft.quantum.guide.host-programs).</span><span class="sxs-lookup"><span data-stu-id="cf10d-109">For additional details, see [Ways to run a :::no-loc(Q#)::: program](xref:microsoft.quantum.guide.host-programs).</span></span>
...
<span data-ttu-id="cf10d-122">Při spuštění :::no-loc(Q#)::: programu z příkazového řádku použijte parametr **--simulátor** (nebo **-s** ) k určení `ResourcesEstimator` cílového počítače.</span><span class="sxs-lookup"><span data-stu-id="cf10d-122">When running a :::no-loc(Q#)::: program from the command line, use the **--simulator** (or **-s** shortcut) parameter to specify the `ResourcesEstimator` target machine.</span></span> <span data-ttu-id="cf10d-123">Následující příkaz spustí program pomocí Estimator prostředků:</span><span class="sxs-lookup"><span data-stu-id="cf10d-123">The following command runs a program using the resources estimator:</span></span> 
...
<span data-ttu-id="cf10d-125">:::no-loc(Q#):::Pro spuštění operace použijte [odhad](xref:microsoft.quantum.iqsharp.magic-ref.simulate) příkazu I Magic% :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="cf10d-125">Use the I:::no-loc(Q#)::: magic command [%estimate](xref:microsoft.quantum.iqsharp.magic-ref.simulate) to run the :::no-loc(Q#)::: operation.</span></span>
...
<span data-ttu-id="cf10d-129">Následující kód ukazuje, jak načíst a vytisknout celkový počet `QubitClifford` `T` `CNOT` operací využívaných :::no-loc(Q#)::: operací:</span><span class="sxs-lookup"><span data-stu-id="cf10d-129">The following code shows how to retrieve and print the total number of `QubitClifford`, `T` and `CNOT` operations used by a :::no-loc(Q#)::: operation:</span></span>
...
|<span data-ttu-id="cf10d-144">__Úrovní__</span><span class="sxs-lookup"><span data-stu-id="cf10d-144">__Depth__</span></span>|<span data-ttu-id="cf10d-145">Hloubka okruhu nečinnosti, kterou spouští :::no-loc(Q#)::: operace (viz [níže](#depth-width-and-qubitcount)).</span><span class="sxs-lookup"><span data-stu-id="cf10d-145">Depth of the quantum circuit run by the :::no-loc(Q#)::: operation (see [below](#depth-width-and-qubitcount)).</span></span> <span data-ttu-id="cf10d-146">Ve výchozím nastavení metrika hloubky počítá jenom `T` brány.</span><span class="sxs-lookup"><span data-stu-id="cf10d-146">By default, the depth metric only counts `T` gates.</span></span> <span data-ttu-id="cf10d-147">Další podrobnosti najdete v tématu s [čítačem hloubky](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).</span><span class="sxs-lookup"><span data-stu-id="cf10d-147">For more details, see [Depth Counter](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).</span></span>   |
|<span data-ttu-id="cf10d-148">__Délk__</span><span class="sxs-lookup"><span data-stu-id="cf10d-148">__Width__</span></span>|<span data-ttu-id="cf10d-149">Šířka okruhu nečinnosti, kterou spouští :::no-loc(Q#)::: operace (viz [níže](#depth-width-and-qubitcount)).</span><span class="sxs-lookup"><span data-stu-id="cf10d-149">Width of the quantum circuit run by the :::no-loc(Q#)::: operation (see [below](#depth-width-and-qubitcount)).</span></span> <span data-ttu-id="cf10d-150">Ve výchozím nastavení metrika hloubky počítá jenom `T` brány.</span><span class="sxs-lookup"><span data-stu-id="cf10d-150">By default, the depth metric only counts `T` gates.</span></span> <span data-ttu-id="cf10d-151">Další podrobnosti najdete v tématu s [čítačem hloubky](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).</span><span class="sxs-lookup"><span data-stu-id="cf10d-151">For more details, see [Depth Counter](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).</span></span>   |
|<span data-ttu-id="cf10d-152">__QubitCount__</span><span class="sxs-lookup"><span data-stu-id="cf10d-152">__QubitCount__</span></span>    |<span data-ttu-id="cf10d-153">Dolní mez pro maximální počet qubits přidělených během běhu :::no-loc(Q#)::: operace.</span><span class="sxs-lookup"><span data-stu-id="cf10d-153">The lower bound for the maximum number of qubits allocated during the run of the :::no-loc(Q#)::: operation.</span></span> <span data-ttu-id="cf10d-154">Tato metrika nemusí být kompatibilní s __hloubkou__ (viz níže).</span><span class="sxs-lookup"><span data-stu-id="cf10d-154">This metric might not be compatible with __Depth__ (see below).</span></span>  |
|<span data-ttu-id="cf10d-155">__BorrowedWidth__</span><span class="sxs-lookup"><span data-stu-id="cf10d-155">__BorrowedWidth__</span></span>    |<span data-ttu-id="cf10d-156">Maximální počet qubits, které byly v rámci operace vypůjčeny :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="cf10d-156">The maximum number of qubits borrowed inside the :::no-loc(Q#)::: operation.</span></span>  |
