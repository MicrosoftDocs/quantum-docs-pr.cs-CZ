...
I když se jedná o triviální příklad (protože operace <xref:microsoft.quantum.intrinsic.h> je samostatně sousedící), můžete zjistit, jak se to u složitějších operací qubit stalo nevýznamem.
Další informace najdete v tématu [operace a funkce](xref:microsoft.quantum.techniques.opsandfunctions).
...
Jinými slovy, sada bran je univerzální branou, pokud může být jakákoli Jednotková transformace přibližně zapsaná jako produkt z brány z této sady. Vyžadujeme, aby pro všechny předepsané chyby byly brány $G _{1}, G_{2}, \ldots, G_N $ z této sady brány.
