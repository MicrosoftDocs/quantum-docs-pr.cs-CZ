...
V případě, že bitová nebo binární číslice může mít hodnotu buď $0 $ nebo $1 $, může qubit mít hodnotu, která je buď z těchto hodnot, nebo je nadmnožinou hodnoty $0 $ a $1 $.
...
I když se jedná o triviální příklad (protože <xref:microsoft.quantum.intrinsic.h> operace je samostatně sousedící), můžete zjistit, jak to bude pro složitější operace qubit nevýznamné.
...
Jinými slovy, sada bran je univerzální branou, pokud může být jakákoli Jednotková transformace přibližně zapsaná jako produkt z brány z této sady. Pro všechny předepsané chyby vyžadujeme, aby v sadě bran existovaly brány $G _ {1} , G_ {2} , \ldots, G_N $.
...
Spolu s bránou Clifford (Brána $T $) můžou tyto operace sestavovat tak, aby se v jednom qubit procházely každou jednotkovou transformaci.
