...
S radostí zavádíme nejnovější aktualizaci do vývojové sady pro všechna ta.  Tato stránka poskytuje přehled nových funkcí jazyka Q # a popisuje, jak [migrovat](#Migration) existující programy q # do nové syntaxe.  Úplný popis jazyka Q [# najdete v](xref:microsoft.quantum.guide.basics)naší kompletní dokumentaci.
...
## <a name="language-features"></a>kompatibility značek (mc:)
...
Podmíněný výraz je výrazem formuláře `condition ? caseTrue | caseFalse` .
Jak je vidět v příkladu výše, je teď také podporována možnost iterace nad poli.

Alokace v rámci `using` a `borrowing` jsou nově možné pro jednotlivá qubits, qubit pole a vnořené řazené kolekce členů této služby:
```qsharp
    using (qubit = Qubit()) {
...
Dvě nové direktivy generátoru specializace `invert` a `distribute` kromě `auto` nich jsou zavedené ke zvýšení kontroly nad tím, jak se generují specializace funktor.
Explicitní deklarace výchozí `body` specializace v rámci operací již není povinná.
...
Modifikátory sváže užší než jakýkoli jiný výraz kombinátorem a zejména v těsném výrazu, než je kombinátorem výrazu volání `(` `)` . 
`Adjoint`, `Controlled` a nové příponový operátor Unwrap `!` je považován za modifikátory výrazu. Ošetření `Adjoint` a `Controlled` jako modifikátory výrazů eliminují potřebu závorek pro velké množství případů. Níže uvedené výrazy jsou platné pro účely přechodu na `Op` určitou, jednotnou operaci, `opArr` pole jednotkových operací a `arg` vhodný argument: 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
Skript migrace vynechá většinu zbytečných závorek a bude plně využívat nové funkce. Nicméně se doporučuje, aby skript neinterpretoval kód, a nebude mít za následek jiný výklad `Controlled` funktor ve výjimečných případech (týká se pouze v kombinaci s částečnými aplikacemi).
...
```
Jak je možné vidět z příkladu, `!` váže užší, než modifikátory předpony, jako `Controlled` a `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>Průlomové změny 
...
Kromě níže uvedených změn je vzor, `__*__` kde * je jakákoli sekvence neprázdných znaků, vyhrazena pro interní použití a nemůže být použit jako název symbolu.  
...
Následující příklad ukazuje, jak použít `ToOperation` funkci ke změně funkce `Square` na operaci `op` : 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>Migrace stávajících projektů na Q # 0,3 
...
2. V textovém editoru upravte soubor. csproj tak, aby se změnila verze všech společností "Microsoft. `PackageReference` prohlášené" na verzi 0.3.1811.2802-Preview, například:
```xml
...
```
4. Z příkazového řádku použijte formátovací nástroj integrovaný do příkazového řádku kompilátoru pro řešení všech změn syntaxe spuštěním tohoto příkazu:`dotnet msbuild /t:qsharpformat`  
...
## <a name="migration-script"></a><a name="MigrationScript"></a>Skript migrace 
...
- Projekt je sestavený s předchozí verzí vývojové sady pro plnění do provozu, aby bylo zajištěno, že migrace bude nejspíš úspěšná. Tento krok se dá přeskočit pomocí `-Force` parametru ke skriptu.
...
> Aby bylo možné spouštět skripty, jako je například `qdk-migrate.ps1` spouštění v systému Windows, může být nutné změnit zásady spouštění.
