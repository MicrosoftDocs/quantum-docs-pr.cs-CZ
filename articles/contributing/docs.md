...
Komentář k dokumentaci by měl pomáhat uživateli, jak interpretovat `bits` a `oracle` a co je funkce pro.
...
Pro příklad `ControlledOnBitString`můžeme napsat něco podobného jako následující:
