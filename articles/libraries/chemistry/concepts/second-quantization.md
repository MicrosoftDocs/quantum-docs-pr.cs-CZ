...
Místo přiřazování každé z $N _e $ Electrons ke konkrétnímu stavu (nebo Orbital) Druhá kvantizační každou Orbital sleduje a ukládá, zda je v každé z nich přítomna elektronická část a ve stejnou dobu automaticky zajišťuje vlastnosti symetrie odpovídající funkce Wave.
...
Jako příklad druhé kvantizační v akci se předpokládá, že $ \psi_0\cdots \psi_{N-1} $ je orthonormal sada prostorových orbitals.
...
To znamená, že platnými základními stavy jsou formuláře $ \psi_{0, \uparrow}, \ldots, \psi_{N-1, \uparrow}, \psi_{0, \downarrow}, \ldots, \psi_{N-1, \downarrow} $ WHERE $ \uparrow $ a $ \downarrow $ jsou popisky, které určují dva eigenstatesy stupně otáčení Svoboda.
...
To znamená, že můžeme zapsat dva právní stavy pro $ \psi_1 $ as \begin{Equation} \psi_1 \rightarrow \begin{Cases} \ket{0}_1 & \Text{if $ \psi_1 $ není obsazené \\\
\ket{1}_1 & \Text{if $ \psi_1 $ je obsazené.} \end{Cases} \end{Equation} toto kódování je skvělé pro počítače, protože to znamená, že je možné uložit elektronické povolání jako jeden bit.
...
\end{align} Všimněte si, že tady $a ^ \dagger_1 \ket{1}_1 = 0 $ a $a _1 \ket{0}_1 $ Dej nulové vektor not $ \ket{0}_1 $.
...
\end{Equation} Tento operátor počítá povolání daného číselníku Orbital, což znamená \begin{align} n_i \ket{0}_i & = 0 \ č.\\\
...
$ $ Tyto operátory jsou označovány jako "" anti-dojíždění "a obecně pro všechny $i, j $ máme \begin{Align} a ^ \dagger_i a ^ \dagger_j =-(1-\delta_{i, j}) ^ \dagger_j a ^ \dagger_i, \quad a ^ \dagger_i a_j = \delta_{i, j}-a_j a ^ \dagger_i.
...
\begin{Equation} \hat{H} = \sum\_{pq} H\_{pq} a ^ \dagger\_p a\_q + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_p a ^ \dagger\_q a\_RA\_s + h\_{\textrm NUC}, \label{EQ: totalHam} \end{Equation} kde $h\_{\textrm NUC} $ je jaderná energie (což je konstanta pod odhadem Oppenheimer) a

\begin{align} h\_{pq} & = \int\_{-\infty} ^ \infty \psi \left\*\_p (x\_1) \frac{\nabla (-\right ^ 2}{2} + V (x\_1) \psi) \mathrm{d}\_q (x\_1) 3x ^ @no__ t_9_ 1, \end{align}

kde $V (x) $ je potenciál v poli střední hodnoty a

\begin{align} h\_{pqrs} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*(x\_1) \psi\_q ^\*(x\_2) \left (\frac{1}{| x_1-x_2 |} \ Right) \psi\_r (x\_2) \psi\_s (x\_1) \mathrm{d} ^ 3x\_1 \ mathrm {d} ^ 3x\_2. \ Label {EQ: integrs} \end{align}

Výrazy $h\_{pq} $ jsou odkazovány jako integrály, protože všechny takové výrazy zahrnují pouze jeden Electrons a podobně $h\_{pqrs} $ jsou dva elektronické integrály.
...
Například $h _ {pq} a ^ \dagger_p a_q $ popisuje Orbital skákající od míchy Orbital $q $ k otočení $p $.
Podobně pojem $h _ {pqrs} a ^ \dagger_p a ^ \dagger_q a_r a_s $ (pro odlišná $p, q, r, s $) popisuje dva Electrons ve míchách orbitals $r $ a $s $ a končí v orbitals $p $ a $q $.
Pokud $r = q $ a $p = s $, pak $h _ {prrp} a ^ \dagger_p a ^ \dagger_r a_r a_p = H_ {prrp} n_p n_r $ poskytuje snížení energie spojené s těmito dvěma electronsy blízko sebe, ale nepopisuje dynamický proces.
...
Zvažte například Hamiltonian $H = A_0 ^ \dagger A_1 ^ \dagger A_1 A_0-A_1 ^ \dagger A_0 ^ \dagger A_1 A_0 = 2a_0 ^ \dagger A_1 ^ \dagger A_1 A_0 $, což je součet výše uvedených podmínek.
...
$ $ \begin{align} H = \sum\_{pq} H\_{pq} a ^ \dagger\_{p} a\_{q} + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} @no__ t_10_ {r}\_{s}.
\end{align} $ $
...
Například v případě, že $p, q, r, s $ jsou jedinečné indexy, můžeme použít pravidla pro boj do provozu k tomu, aby ukázala, že: $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r}\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{ p}\_{r}\_{s} =-a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{s}\_{r} = ^ \dagger\_{q} a ^ \dagger\_{p}\_{r}.
$$

Kromě toho, když je $H $ Hermitian, každý Hermitian fermionic operátor, řekněme $h\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} $, má Hermitian, který se nachází také v $H $. Aby bylo možné jedinečně indexovat skupiny podmínek, které jsou charakterizovány těmito symmetries, definujeme kanonické řazení pro indexy $ (i\_1, \cdots, i\_n, j\_1, \cdots, j\_m) $ z jakékoli sekvence $n + m $ fermionic Operators $a ^ \dagger\_{i\_1} \cdots a ^ \dagger\_{i\_n}\_{j\_1} \cdots a\_{j\_m} $as následující:
...
V tomto kanonickém řazení může být fermionic Hamiltonian vyjádřena jako $ $ \begin{align} H = \sum\_{(p, q) \in S\_{1,1}} H '\_{pq} \frac{a ^ \dagger\_{p}\_{q} + a ^ \dagger\_{q}\_p}}{2}+ \sum\_{(p, q, r, s) \in S\_{2,2}} h '\_{pqrs} \frac{a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r}\_{s} + a ^ \dagger\_{s} a ^ \ Dagger\_{r}\_{q} a\_{p}}{2}, \end{align} $ $ s vhodně přizpůsobeným jedním a dvěma e-integrály $h\_{pq} $ a $h\_{pqrs} $ v uvedeném pořadí.
