...
1. **`LittleEndian`**: Pole `qArr : Qubit[]` qubit, které představuje celé číslo, `qArr[0]` kde označuje nejméně významný bit.
...
1. **`FixedPoint`**: Představuje reálné číslo sestávající z pole `qArr2 : Qubit[]` qubit a umístění `pos`binárního bodu, které počítá počet binárních číslic nalevo od binárního bodu. `qArr2`je uložen stejným způsobem jako `SignedLittleEndian`.
...
Aby bylo možné vyhodnotit hladké funkce, jako je $ \sin (x) $ $x, na počítači s `FixedPoint` více `EvaluatePolynomialFxP` `Evaluate[Even/Odd]PolynomialFxP`procesory

První, `EvaluatePolynomialFxP`,,, umožňuje vyhodnotit polynomu ve tvaru $ $ P (x) = a_0 + a_1x + a_2x ^ 2 + \cdots + a_dx ^ d, $ $, kde $d $ označuje *stupeň*. To provedete tak, že všechno, co je potřeba, jsou `[a_0,..., a_d]` polynomické koeficienty (typu `Double[]`) `x : FixedPoint` , vstup a `y : FixedPoint` výstup (zpočátku nula):
```qsharp
